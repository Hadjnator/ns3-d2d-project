...
static double simulateur(int argc, char* argv[], double Current_ueY, double simulationDuration){
    uint32_t mcs = 13;
    ...
    double ueY = Current_ueY;

    double simTime = simulationDuration; // 50 seconds: to take statistics
    ...
    std::cout << Current_ueY << std::endl;
    return sum / packetsTime.size();
}

static double* mainSimulateur(int argc, char* argv[], double* ue_pos, int size, double simulationDuration){
    double* averageLatencies = new double[size];
    for(int i=0; i<size; i++){
        averageLatencies[i] = simulateur(argc, argv, ue_pos[i], simulationDuration);
    }
    return averageLatencies;
}

static void afficheResult(double* table, int size){
    std::cout << "************************************" << std::endl;
    for(int i=0; i<size; i++){
...
void write_csv(std::string filepath, double* ue_pos, double* averageLatencies, int size){
    std::ofstream file;
    file.open(filepath);
    for(int i=0; i<size; i++){
        file << ue_pos[i] << ',' << averageLatencies[i] << '\n';
    }
    file.close();
}
...
main(int argc, char* argv[])
{
    ...
    double ue_pos[] = {10, 20, 30, 40};
    int size = 4;
    double* result = mainSimulateur(argc, argv, ue_pos, size, simulationDuration);
    afficheResult(result, size);
    write_csv("output.csv", ue_pos, result, size);
    return 0;
}
